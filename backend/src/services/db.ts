 ...
 * @throws {Error} If the database has not been initialized via initDb()
 */
...
/**
 * Initializes the SQLite database with WAL mode, pragmas, and schema migrations.
...
/**
 * Creates all tables, indexes, and virtual tables if they do not exist.
 * Applies incremental schema migrations for backwards-compatible column additions.
 */
...
/**
 * Synchronizes the full-text search (FTS5) index for a stream record.
 * @param streamId - The stream's unique identifier
 * @param sender - The sender's Stellar account ID
 * @param recipient - The recipient's Stellar account ID
 * @param assetCode - The asset code for the stream
 */
...
/**
 * Searches the full-text search index and returns matching stream IDs.
 * @param query - The FTS5 query string
 * @returns An array of matching stream ID strings, sorted by relevance rank
 */
